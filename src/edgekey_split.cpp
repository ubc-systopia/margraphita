...
void SplitEdgeKey::create_wt_tables(graph_opts &opts, WT_CONNECTION *conn)
{
  ...
  if (opts.prop_mode == COLUMNAR)
  {
    ...
    ret = sess->create(sess, ("colgroup:" + PERSON_PROPS_TABLE + ":" + CG_LOCATION).c_str(),
        "columns=(country_id)");
    if (ret != 0)
      throw GraphException("Failed to create person_props:location colgroup: " + std::string(wiredtiger_strerror(ret)));
...
int SplitEdgeKey::add_node(node to_insert, bool is_bulk)
{
  ...
  if (out_edge_cursor->search(out_edge_cursor) == 0)
  {
    ...
    if (int ret = error_check_insert_txn(out_edge_cursor->insert(out_edge_cursor)))
      return ret;
  }
  if (int ret = error_check_insert_txn(session->commit_transaction(session, nullptr)))
    return ret;
  ...
  if (opts.prop_mode == COLUMNAR)
  {
    switch (VTYPE_OF(id))
    {
      case VT_PERSON: {
        ...
        uint64_t    country_id = (prop_size >= SNBPersonSchema::TOTAL_SIZE)
                                  ? SNBPersonSchema::get_country_id(prop_data) : 0;
...
std::vector<node_id_t> SplitEdgeKey::get_in_nodes_id(node_id_t node_id)
{
  ...
  int search_exact;
  CommonUtil::ekey_set_node_key(in_cur, node_id);
  if (in_cur->search_near(in_cur, &search_exact) == WT_NOTFOUND)
  {
    in_cur->reset(in_cur);
    return in_nodes_id;  // table empty or cursor past end — no in-edges
  }
  if (search_exact <= 0)
  {
    ...
    {
      in_cur->reset(in_cur);
      return in_nodes_id;  // no entries after search position
    }
  }
