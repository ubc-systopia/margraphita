...
node EdgeKey::get_node(node_id_t node_id)
{
  ...
  // OutOfBand_ID_MIN);
  CommonUtil::ekey_set_key(edge_cursor, node_id, OutOfBand_ID_MIN);
  /////////////////////////////////////////////////////////////////////////
...
node EdgeKey::get_random_node()
{
  ...
  {
    ret = _get_table_cursor(EDGE_TABLE, &random_cursor, session, true, true);
  }
  if (ret != 0)
  {
...
int EdgeKey::add_node(node to_insert, bool is_bulk)
{
  ...
  // OutOfBand_ID_MIN);
  CommonUtil::ekey_set_key(edge_cursor, to_insert.id, OutOfBand_ID_MIN);
  /////////////////////////////////////////////////////////////////////////
...
int EdgeKey::add_node_txn(node to_insert)
{
  ...
  {
    edge_cursor->set_value(edge_cursor, 0, OutOfBand_ID_MAX);
  }
                                           ...
                                           int *num_edges_to_add)  // TODO
{
  ...
  while (del_cursor->next(del_cursor) == 0)
  {
    ...
    {
      session->rollback_transaction(session, nullptr);
      throw GraphException("Failed to remove edge between " + to_string(src) +
                           " and " + to_string(dst) + wiredtiger_strerror(ret));
    }
    else
    ...
    if (opts.read_optimize)
    {
      ...
      /////////////////////////////////////////////////////////////////////////
      ret = update_node_degree(temp.id, temp.in_degree - 1, temp.out_degree);
      if (ret != 0)
      {
        session->rollback_transaction(session, nullptr);
  ...
  if (opts.read_optimize)
  {
    ...
    // OutOfBand_ID_MIN);
    CommonUtil::ekey_set_key(dst_src_idx_cursor, node_id, OutOfBand_ID_MIN);
    /////////////////////////////////////////////////////////////////////////
    int exact_match;
    ...
    else if (exact_match > 0) /* We cannot have an exact match on the dst,src
                               ...
                               index. (see the dump of the index if in doubt)*/
    {
      ...
      do
      {
        ...
        ret = update_node_degree(temp.id, temp.in_degree, temp.out_degree - 1);
        if (ret != 0)
        {
          session->rollback_transaction(session, nullptr);
          throw GraphException("Could not update the node with ID" +
        ...
        if (ret != 0)
        {
          ...
          throw GraphException("Failed to remove edge between " +
                               to_string(src_) + " and " + to_string(dst_) +
                               wiredtiger_strerror(ret));
        }
        else
        {
  ...
  if (ret != 0)
  {
    ...
    throw GraphException("Failed to remove node with ID " +
                         std::to_string(node_id) + wiredtiger_strerror(ret));
  }
  edge_cursor->reset(edge_cursor);
  return 0;
}
...
int EdgeKey::add_edge_only(edge to_insert)
{
  ...
  {
    edge_cursor->set_value(
        edge_cursor, to_insert.edge_weight, OutOfBand_ID_MAX);
  }
  else
  {
    edge_cursor->set_value(edge_cursor, 0, OutOfBand_ID_MAX);
...
int EdgeKey::delete_edge(node_id_t src_id, node_id_t dst_id)  // TODO
{
  ...
  switch (ret)
  {
    ...
    default:
      ...
      throw GraphException("Failed to delete the edge between " +
                           ...
                           std::to_string(dst_id) + wiredtiger_strerror(ret));
  }
  ...
  if (!opts.is_directed)
  {
    ...
    switch (ret)
    {
      ...
      default:
        session->rollback_transaction(session, nullptr);
        throw GraphException("Failed to delete the reverse edge between " +
                             std::to_string(src_id) + " and " +
                             std::to_string(dst_id) + wiredtiger_strerror(ret));
    }
  }
  ...
  if (opts.read_optimize)
  {
    ...
    switch (ret)
    {
      ...
      default:
        session->rollback_transaction(session, nullptr);
        throw GraphException("Could not update the node with ID" +
                             std::to_string(src.id) + wiredtiger_strerror(ret));
    }
...
edge EdgeKey::get_edge(node_id_t src_id, node_id_t dst_id)
{
  ...
  if (edge_cursor->search(edge_cursor) == 0)
  {
    found.src_id = src_id;
    found.dst_id = dst_id;
    if (opts.is_weighted) CommonUtil::record_to_edge_ekey(edge_cursor, &found);
  }
  return found;
}
  ...
  if (ret == 0)  // The node exists
  {
    ...
    else
    {
      ...
      do
      {
        ...
        //} while (src == MAKE_EKEY(node_id) && dst !=
        // OutOfBand_ID_MIN);
...
std::vector<node> EdgeKey::get_out_nodes(node_id_t node_id)
{
  ...
  /////////////////////////////////////////////////////////////////////////
  // CommonUtil::ekey_set_key(e_cur, MAKE_EKEY(node_id), OutOfBand_ID_MIN);
...
WT_CURSOR *EdgeKey::get_new_random_node_cursor()
{
  ...
  if (_get_table_cursor(EDGE_TABLE, &random_node_cursor, session, true, true) !=
      0)
  {
...
WT_CURSOR *EdgeKey::get_dst_src_idx_cursor()
{
  if (dst_src_idx_cursor == nullptr)
  {
    ...
    if (_get_index_cursor(
            EDGE_TABLE, DST_SRC_INDEX, projection, &dst_src_idx_cursor) != 0)
    {
...
WT_CURSOR *EdgeKey::get_new_dst_src_idx_cursor()
{
  ...
  string projection = "(" + ATTR_FIRST + "," + ATTR_SECOND + ")";
  if (_get_index_cursor(
          EDGE_TABLE, DST_SRC_INDEX, projection, &new_dst_src_idx_cursor) != 0)
  {
    throw GraphException("Could not get a cursor to DST_SRC_INDEX");
  }
...
InCursor *EdgeKey::get_innbd_iter()
{
  ...
  InCursor *toReturn = new EkeyInCursor(get_new_dst_src_idx_cursor(), session);
...
WT_CURSOR *EdgeKey::get_edge_cursor()
{
  ...
  {
    if (_get_table_cursor(EDGE_TABLE, &edge_cursor, session, false, true) != 0)
    {
      throw GraphException("Could not get a cursor to the Edge table");
    }
  }
