 ...
 public:
  ...
  void set_key_range(key_range _keys) override
  {
    ...
    CommonUtil::set_key(cursor,
                        keys.start);  // set the key to the start of the range
    int status;
  ...
  void next(adjlist *found) final
  {
    ...
    if (keys.end != OutOfBand_ID_MAX && dst > keys.end)
    {
      goto no_next;
    }
...
class StdNodeCursor : public NodeCursor
{
 public:
  ...
  void no_next(node *found)
  {
    ...
    found->in_degree = UINT32_MAX;   // always u32
    found->out_degree = UINT32_MAX;  // always u32
    has_next = false;
  }
...
class StdEdgeCursor : public EdgeCursor
{
 public:
  ...
  void no_next(edge *found)
  {
    ...
    found->edge_weight = UINT32_MAX;  // always u32
    has_next = false;
  }
...
class StandardGraph : public GraphBase
{
 public:
  ...
  void get_random_node_ids(std::vector<node_id_t> &nodes,
