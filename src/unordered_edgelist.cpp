 ...
 */
//! Should work as is -- copied from AdjList. No need to change
...
void UnOrderedEdgeList::create_wt_tables(graph_opts &opts, WT_CONNECTION *conn)
{
    ...
    string edge_key_format =
        "u";  // 64 bit int, big_endian using __builtin_bswap64
    string edge_value_format = "II";  // source, dst, unsigned ints
    if (opts.is_weighted)
    {
...
int UnOrderedEdgeList::add_node_in_txn(node to_insert)
{
    int ret;
    CommonUtil::set_key(node_cursor, to_insert.id);
 ...
 */
int UnOrderedEdgeList::delete_node(node_id_t node_id)
{
    // Delete Edges
 ...
 */
uint32_t UnOrderedEdgeList::get_in_degree(node_id_t node_id)
{
    int ret;
    if (opts.read_optimize)
    {
 ...
 */
uint32_t UnOrderedEdgeList::get_out_degree(node_id_t node_id)
{
    node_cursor->reset(node_cursor);
    if (opts.read_optimize)
    {
 ...
 */
[[maybe_unused]] void UnOrderedEdgeList::update_edge_weight(node_id_t src_id,
                                                            node_id_t dst_id,
                                                            int32_t edge_weight)
{
    if (!opts.is_weighted)
 ...
 * @return std::vector<int> The UnOrderedEdgeList for the node.
 */
std::vector<node_id_t> UnOrderedEdgeList::get_adjlist(WT_CURSOR *cursor,
                                                      node_id_t node_id)
{
    int ret;
    adjlist adj_list;
...
int UnOrderedEdgeList::add_to_adjlists(WT_CURSOR *cursor,
                                       node_id_t node_id,
                                       node_id_t to_insert)
{
...
int UnOrderedEdgeList::delete_from_adjlists(WT_CURSOR *cursor,
                                            node_id_t node_id,
                                            node_id_t to_delete)
{
...
[[maybe_unused]] void UnOrderedEdgeList::delete_node_from_adjlists(
    node_id_t node_id)
{
    ...
    {
        throw GraphException("Could not find " + std::to_string(node_id) +
                             " in the UnOrderedEdgeList Table");
    }
 ...
 */
int UnOrderedEdgeList::delete_related_edges_and_adjlists(node_id_t node_id,
...
WT_CURSOR *UnOrderedEdgeList::get_in_adjlist_cursor()
{
...
WT_CURSOR *UnOrderedEdgeList::get_new_in_adjlist_cursor()
{
...
WT_CURSOR *UnOrderedEdgeList::get_out_adjlist_cursor()
{
...
WT_CURSOR *UnOrderedEdgeList::get_new_out_adjlist_cursor()
{
...
[[maybe_unused]] node UnOrderedEdgeList::get_next_node(WT_CURSOR *n_cur)
{
    node found = {0};
...
[[maybe_unused]] edge UnOrderedEdgeList::get_next_edge(WT_CURSOR *e_cur)
{
    edge found = {0};
...
[[maybe_unused]] void UnOrderedEdgeList::dump_tables()
{
    cout << "------\n"
         << "Nodes:\n"
         << "------\n";
...
int UnOrderedEdgeList::add_one_node_degree(WT_CURSOR *cursor,
                                           node_id_t to_update,
                                           bool is_out_degree)
{
...
int UnOrderedEdgeList::remove_one_node_degree(node_id_t to_update,
                                              bool is_out_degree)
{
    int ret;
...
int UnOrderedEdgeList::delete_edge_in_txn(node_id_t src_id, node_id_t dst_id)
{
    int ret = 0;
    CommonUtil::set_key(edge_cursor, src_id, dst_id);
    if ((ret = error_check_remove_txn(edge_cursor->remove(edge_cursor))))
