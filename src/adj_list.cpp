...
void AdjList::init_cursors()
{
    ...
    if ((ret =
             _get_table_cursor(EDGE_TABLE, &edge_cursor, session, false, true)))
    {
                                ...
                                int32_t outdeg_change)
{
    ...
    switch (ret)
    {
        ...
        case WT_ROLLBACK:
            LOG_MSG(
                "Failed to update node degree for node {}; TX rolled back: {}",
                node_id,
                wiredtiger_strerror(ret));
            return WT_ROLLBACK;
        default:
                                               ...
                                               int *num_edges_deleted)
{
    ...
    *num_edges_deleted = 0;
    ...
    // Get outgoing nodes
    std::vector<node_id_t> out_nodes =
        get_adjlist(out_adjlist_cursor, to_delete);
    ...
    {
        // Get incoming nodes
        std::vector<node_id_t> in_nodes =
            get_adjlist(in_adjlist_cursor, to_delete);
