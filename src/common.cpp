 ...
 */
std::vector<int> CommonUtil::get_default_nonstring_attrs(std::string fmt)
{
    std::vector<int> ret;
 ...
 */
std::vector<std::string> CommonUtil::get_default_string_attrs(std::string fmt)
{
                                             ...
                                             size_t *total_size)
{
    ...
    for (std::vector<int>::iterator iter = to_pack.begin();
         iter != to_pack.end();
         ++iter)
    {
        fmt = fmt + 'i';
...
std::vector<std::string> CommonUtil::unpack_string_vector_std(
    std::string packed_str)
{
    ...
    std::string token;
    while ((packed_str != "") &&
           ((pos = packed_str.find(delimiter)) != std::string::npos))
    {
 ...
 */
std::vector<std::string> CommonUtil::unpack_string_vector_wt(
    const char *to_unpack, WT_SESSION *session)
{
    ...
    for (uint i = 0; i < fmt_len; i++)
    {
        const char *res = (char *)malloc(
            50 * sizeof(char));  //! I don't know a good size to use here and
                                 //! don't know how to estimate this either.
        if (i == 1)
        {
            int64_t temp;
 ...
 * saved to the packing stream is the format string itself.
 */
char *CommonUtil::pack_int_vector_wt(std::vector<int> to_pack,
                                     WT_SESSION *session,
                                     size_t *size,
                                     std::string *fmt)
{
 ...
 */
std::string CommonUtil::pack_int_vector_std(std::vector<int> to_pack,
                                            size_t *size)
{
    size_t _size = 0;
    std::string buffer;
 ...
 * @return std::vector<int> unpacked buffer
 */
std::vector<int> CommonUtil::unpack_int_vector_wt(const char *to_unpack,
                                                  WT_SESSION *session)
{
...
char *CommonUtil::pack_string_wt(std::string to_pack,
                                 WT_SESSION *session,
                                 std::string *fmt)
{
    // std::string format = std::to_string(to_pack.length()) + "s";
    // ? Fix this if things go wrong. Does packing a string require the length?
    // I
    // //? don't think so.
    std::string format = "S";
    size_t size;
...
{
    const char *fmt = "S";
    const char *buffer = (char *)malloc(
        50 * sizeof(char));  // ! Fix this Again, random guess of buffer size
...
char *CommonUtil::pack_int_wt(int to_pack, WT_SESSION *session)
{
    ...
    (void)wiredtiger_pack_start(
        session, format.c_str(), buffer, sizeof(int), &psp);
 ...
 * @return char* packed buffer.
 */
char *CommonUtil::pack_bool_wt(bool to_pack,
                               WT_SESSION *session,
                               std::string *fmt)
{
...
bool CommonUtil::unpack_bool_wt(const char *to_unpack, WT_SESSION *session)
{
    ...
    (void)wiredtiger_unpack_start(
        session, "i", to_unpack, sizeof(uint64_t), &ps);
...
int CommonUtil::open_session(WT_CONNECTION *conn, WT_SESSION **session)
{
    ...
    // }
    // return 0;
    if (conn->open_session(conn, NULL, "isolation=read-uncommitted", session) !=
        0)
    {
        fprintf(stderr, "Failed to open session\n");
        return (-1);
 ...
 * @return WT_ITEM
 */
WT_ITEM CommonUtil::pack_vector_string(WT_SESSION *session,
 ...
 * @param to_pack the vector to pack
 * @param fmt format string will hold the vector description. This is set from
 * this function.
 * @return WT_ITEM
 */
WT_ITEM CommonUtil::pack_vector_int(WT_SESSION *session,
                                    ...
                                    std::string *fmt)
{
    ...
    wiredtiger_pack_start(
        session, format.c_str(), data_buf, size, &pack_stream);
 ...
 * @param ... All the items that need to be packed. Strings should be passed as
 * char*
...
std::vector<int> CommonUtil::unpack_vector_int(WT_SESSION *session,
                                               WT_ITEM packed,
                                               std::string format)
{
