...
{
    WT_CURSOR *edge_cur;
    CommonUtil::_get_table_cursor("edge", &edge_cursor, session, false, false);
    for (int i = 0; i < 1e8; i++)
    {
        if (i % 2 == 0)
        {
...
#pragma omp parallel for num_threads(THREAD_NUM)
    for (int t = 0; t < THREAD_NUM; t++)
    {
        ...
        conn->open_session(conn, NULL, NULL, &session);
        WT_CURSOR *edge_cur;
        CommonUtil::_get_table_cursor(
            "edge", &edge_cursor, session, false, false);
        for (int i = 0; i < 1e4; i++)
        {
...
#pragma omp parallel for num_threads(THREAD_NUM)
    for (int t = 0; t < THREAD_NUM; t++)
    {
        ...
        for (int i = 0; i < 1e4; i++)
        {
            CommonUtil::set_key(edge_cur, 1e8 + i * 2 + 1);
            edge_cur->set_value(edge_cur, 0, 0);
            edge_cur->insert(edge_cur);
            edge_cur->reset(edge_cur);
        }
