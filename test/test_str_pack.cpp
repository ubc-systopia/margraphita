...
int get_table_cursor(string table, WT_CURSOR **cursor, bool is_random)
{
...
void convert_vector_to_buf(std::vector<int> to_pack)
{
  char *buf = (char *)malloc(to_pack.size() * sizeof(int));
  std::copy(to_pack.begin(), to_pack.end(), buf);
  printf("%s\n", buf);
}
...
std::vector<int> unpack_int_vector(char *to_unpack, WT_SESSION *session)
{
...
int main()
{
  ...
  vect1.push_back(20);
  vect1.push_back(30);
  // keeping the packing buffer size the same, this fails if the vector values
  // are changed to 100, 200, 300

  convert_vector_to_buf(vect1);
