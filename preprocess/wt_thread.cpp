...
std::unordered_map<int, node> parse_node_entries(std::string filename)
{
    ...
    for (std::string file : filenames)
    {
        ...
        if (newfile.is_open())
        {
            ...
            while (getline(newfile, tp))
            {
                if ((tp.compare(0, 1, "#") == 0) or
                    (tp.compare(0, 1, "%") == 0) or (tp.empty()))
                {
                    continue;
                }
...
void *insert_edge_thread(void *arg)
{
    ...
    std::string of = "part";
    of.push_back(c);
    std::ofstream writef(of, std::ios::trunc);
    if (writef.is_open())
    {
        for (edge e : edjlist)
        {
            writef << e.src_id << "\t" << e.dst_id << "\n";
        }
    }
    writef.close();
    ...
    std::cout << tid << "\t" << filename << "\t" << (edge_per_part * tid) + 1
              << "\t" << start_idx << "\t" << size << std::endl;
    delete (int *)arg;
...
int insert_node()
{
    ...
    std::unordered_map<int, node> nodelist =
        reader::parse_node_entries(filename);
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "d:e:n:f:r", long_opts, &option_idx)) !=
           -1)
    {
        switch (c)
        {
            ...
            case '?':
            default:
                std::cout << "enter dbname(--db), edgecount (--edges), "
