...
class Preprocess:
    ...
    def preprocess(self):
        ...
        # Split step removed: mk_adjlists now partitions the input file
        # internally using byte-range reads.
        ...
        cmd = (
            f"{self.config_data['cmd_root']}/preprocess/mk_adjlists_new -d {self.config_data['dataset_name']} -e {self.config_data['num_edges']} "
    ...
    def cleanup(self):
        # Remove the adjlist files produced by mk_adjlists
        self.log("Cleaning up")
        for prefix in ("out", "in", "out_weights", "in_weights"):
            cmd = f"rm -f {self.config_data['output_dir']}/{prefix}_??"
            self.log(f"Running command: {cmd}\n")
            if not self.config_data['dry_run']:
                os.system(cmd)
