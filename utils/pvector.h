...
#pragma omp parallel for
        for (size_t i = 0; i < capacity(); i++) start_[i] = copy_begin[i];
    }
...
#pragma omp parallel for
    ...
    // prevents internal storage from being freed when this pvector is
    // desctructed
    // - used by Builder to reuse an EdgeList's space for in-place graph
    // building
