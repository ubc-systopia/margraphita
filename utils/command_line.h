...
class CmdLineBase
{
 protected:
                        ...
                        const std::string &text)
  {
    ...
    fmt::format_to(
        std::back_inserter(buf), " -{0} {1:20}: {2:100}", opt, opt_arg, text);
 ...
 public:
  CmdLineBase(int argc, char **argv) : argc_(argc), argv_(argv)
  {
    ...
    add_help_message(
        'd', "directed", "(Optional) Is the graph directed. Default = false.");
    add_help_message(
        'w', "weighted", "(Optional) Is the graph weighted. Default = false");
    add_help_message('l',
                     "Log dir",
  ...
  bool parse_args()
  {
    ...
    {
      std::cout << "No specified database path. Use -h for help." << std::endl;
      return false;
    }
...
{
 public:
  SSSPOpts(int argc, char **argv, edgeweight_t _delta) : CmdLineApp(argc, argv)
  {
    argstr_ += "D:";
