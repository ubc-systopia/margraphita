...
protected:
    ...
    {
        char buf[100];
        snprintf(buf, 100, " -%c %-20s: %-54s", opt, opt_arg.c_str(), text.c_str());
...
public:
    CmdLineBase(int argc, char **argv) : argc_(argc), argv_(argv)
    {
        ...
        add_help_message('r', "read_optimize", "Is the DB read optimized. Default true");
    ...
    bool parse_args()
    {
        ...
        if (graph_type == "")
        {
            std::cout << "Please specify the graph representation being used for this benchmark. Use -h for help." << std::endl;
            return false;
        }
        return true;
    }
