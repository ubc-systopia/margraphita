...
struct Bin
{
    ...
    {
        _bin = (uint64_t *)calloc(_size, sizeof(u_int64_t));
        memset(_bin, 0, _size * sizeof(u_int64_t));
        _idx = 0;
        _state = 0;
    }
    ...
    void mark_binning() { atomic_store(&_state, 0); }

    void mark_accumulate() { atomic_store(&_state, 1); }

    bool is_accumulate() { return atomic_load(&_state); }

    void make_empty() { _idx = 0; }

    bool empty() { return _idx == 0; }
    ...
    uint64_t *get_bin() const { return _bin; }

    int get_bin_id() const { return _id; }
...
struct FullBins
{
    ...
    void push(Bin *bin) { _queue.enqueue(bin); }

    Bin *pop()
    {
        Bin *bin;
        if (!_queue.try_dequeue(bin)) return nullptr;
        return bin;
    }
};
...
struct BinPair
{
    ...
    void switch_bin(int count)
    {
        ...
        if (mybin->_idx + count < mybin->_size) return;
...
struct Bins
{
    ...
    void init_buffer()
    {
        int buf_size = _bin_count * _bin_buf_size;
        ...
        {
            _buf[i] = (uint64_t *)calloc(buf_size, sizeof(u_int64_t));
    ...
    void print()
    {
        ...
        printf("bin size: %lu MB = %d * %lu kB bins\n",
               ...
               _bin_count,
               (_bin_size * 8) >> 10);
        size_t buf_size = _bin_count * _bin_buf_size * 8;
        printf("buffer size: %lu KB\n", (buf_size * _nthreads) >> 10);
    }

    Bin *get_full_bin() { return _full_bins.pop(); }

    inline __attribute__((always_inline)) void append(unsigned tid,
                                                      uint32_t x1,
                                                      uint32_t x2)
    {  // x1 and x2 types does not hold within Flexigraph system (x1 is
       // uint64_t)
        // calculate bin index
        ...
        // insert entry
        uint64_t entry = ((uint64_t)x1 << 32) + x2;
    ...
    inline __attribute__((always_inline)) void flush(unsigned tid)
    {
    ...
    inline __attribute__((always_inline)) void flush_all()
    {
        for (int bid = 0; bid < _bin_count; bid++)
        {
    ...
    uint64_t get_bin_size() const { return _bin_size; }

    float get_binning_ratio() const { return _binning_ratio; }
